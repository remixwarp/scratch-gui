...
/**
 * 二次加载加速缓存的版本号。
 * 我们把「已经展开、运行时就绪」的积木表（即 scratch-vm 中 target.blocks._blocks）
 * 直接存进 .rj 的 blocks/prebuilt/<index>.json，下次加载时跳过
 * deserializeBlocks + 逐积木 createBlock，直接当 _blocks 用。
 * 一旦 scratch-vm 的积木内部结构（_blocks 形态）发生变化，就在这里 +1，
 * 旧缓存会被 loader 判定为失效并回退到标准路径，保证向后兼容。
 */
export const RJ_FASTLOAD_VERSION = 1;

/**
 * 二次加载加速缓存：每个角色一份「已展开积木表」的存储路径。
 * @param {number} index 角色序号
 * @returns {string} 该角色预构建积木表的相对路径
 */
export const prebuiltBlockFile = index => `${BLOCK_DIR}/prebuilt/${index}.json`;
