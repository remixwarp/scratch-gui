...
export const buildRJZip = (parts, assets, title) => {
    ...
    // 记录二次加载加速缓存（已展开积木表），并写入版本号以便 loader 校验。
    if (parts.prebuiltFiles && parts.prebuiltFiles.length) {
        manifest.files = {...manifest.files, prebuilt: parts.prebuiltFiles.map(f => f.path)};
        manifest.fastloadVersion = RJ_FASTLOAD_VERSION;
    }
    ...
    for (const file of (parts.prebuiltFiles || [])) {
        zip.file(file.path, JSON.stringify(file.content));
    }
...
export const saveProjectAsRJ = async (vm, title) => {
    ...
    // 二次加载加速缓存：把每个角色「已经展开、运行时就绪」的积木表
    // （即 scratch-vm 里 target.blocks._blocks）直接写进 .rj。下次加载时命中
    // 该缓存即可跳过 deserializeBlocks + 逐积木 createBlock，显著加速大作品。
    // 标准压缩积木（blocks/<i>.json）仍然保留，作为缓存失效时的回退路径，
    // 因此即使老版本打不开 prebuilt 也始终能正常加载。
    try {
        const origTargets = (vm.runtime && vm.runtime.targets) ?
            vm.runtime.targets.filter(t => t.isOriginal) : [];
        const prebuiltFiles = origTargets.map((t, index) => ({
            path: prebuiltBlockFile(index),
            content: {
                index,
                version: RJ_FASTLOAD_VERSION,
                blockCount: (t.blocks && t.blocks._blocks) ? Object.keys(t.blocks._blocks).length : 0,
                blocks: (t.blocks && t.blocks._blocks) ? t.blocks._blocks : {}
            }
        }));
        if (prebuiltFiles.length) {
            parts.prebuiltFiles = prebuiltFiles;
        }
    } catch (e) {
        // 缓存生成失败绝不应影响正常保存，忽略即可。
        // eslint-disable-next-line no-console
        console.warn('[rj] 生成二次加载缓存失败，已忽略：', e);
    }
