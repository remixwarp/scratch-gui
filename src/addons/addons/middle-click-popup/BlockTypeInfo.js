...
export class BlockInstance {
    ...
    /** @type {BlockTypeInfo} */
        ...
        for (let i = 0; i < this.typeInfo.inputs.length; i++) {
            if (this.inputs[i] === null || typeof this.inputs[i] === 'undefined') {
                this.inputs[i] = this.typeInfo.inputs[i].defaultValue;
            }
        }
    }
...
export class BlockInput {
    /**
   * @param {BlockInputType} type The type of input
   * @param {number} inputIdx The index of this input in the workspace version of the block's input array.
   * @param {number} fieldIdx The index of this input in the workspace version of the block's field array.
   */
   ...
   * @param {BlockInstance} block The block to set the value on
   ...
   * @param {BlockInstance} block The block to get the input from
   * @returns {*} The input
   * @protected
   */
    getInput (block) {
   ...
   * @param {BlockInstance} block The block to get the field from
   * @returns {*} The field
   * @protected
   */
    getField (block) {
...
export class BlockInputEnum extends BlockInput {
   ...
   * @param {number} inputIdx The index of this input in the workspace version of the block's input array.
   * @param {number} fieldIdx The index of this input in the workspace version of the block's field array.
   * @param {boolean} isRound Whether this input is a round input
   */
     ...
     * Sets the field this input refers to on a block to a value.
      * @param {BlockInstance} block The block to set the value on
      * @param {BlockInputEnumOption} value The value to set
      */
    setValue (block, value) {
...
export class BlockTypeInfo {
   ...
   */
    ...
    static _createBlocks (workspace, vm, Blockly, locale, workspaceForm, domForm) {
        ...
        if (workspaceForm.id === 'of') {
            ...
            let baseVarInputIdx; let baseTargetInputIdx;
            ...
            const baseVarInput = inputs[baseVarInputIdx];
            const baseTargetInput = inputs[baseTargetInputIdx];
            ...
            for (const targetInput of baseTargetInput.values) {
                ...
                const ofInputs = [];
                ofInputs[baseVarInputIdx] = new BlockInputEnum(
                    options, baseVarInput.inputIdx, baseVarInput.fieldIdx, false
                );
            ...
            const otherScriptsOptionIdx = oldInput.values
                .findIndex(option => option.string === 'other scripts in sprite');
            ...
            return [
                new BlockTypeInfo(workspace, Blockly, vm, workspaceForm, domForm, parts,
                    inputs, BlockShape.End),
                new BlockTypeInfo(workspace, Blockly, vm, workspaceForm, domForm, newBlockParts,
                    [newInput], BlockShape.Stack)
            ];
        }
   ...
   * @param  {...any} inputs The inputs to the block
   * @returns {BlockInstance} The block
   */
    createBlock (...inputs) {
