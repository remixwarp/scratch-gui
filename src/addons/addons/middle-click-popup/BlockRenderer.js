...
const BlockShapes = {
     ...
     */
        snugglePadding: 0,
        get snuggleWith () {
            // BlockShapes.Round doesnt have anyone to snuggle with except itself
            // i dont have this issue cos my gf is awesome :3
 ...
 * @param {BlockShape} shape The shape enum.
 * @param {boolean} isVertical Whether the block is vertical.
 * @returns {BlockComponent} The block shape info.
 */
const getShapeInfo = (shape, isVertical) => {
...
/**
 * @param {BlockInstance} block The block to get the height of.
 ...
 * @param {string} fillVar The fill variable to use.
 * @param {SVGElement} container The element to add the text to.
 * @returns {BlockComponent} The BlockComponent.
 */
const createTextComponent = (text, fillVar, container) => {
    const textElement = container.appendChild(document.createElementNS(SVG_NS, 'text'));
 ...
 * @param {object} shape An object with information of the shape of the new block. From the {@link BlockShapes} object.
 ...
 * @returns {BlockComponent} The block component.
 */
const createBlockComponent = (container, shape, categoryClass, fill, stroke, width) => {
...
const createBackedTextedComponent = (text, container, shape, categoryClass, fill, stroke, textVar) => {
 ...
 * @param {BlockInstance} block The block to render
 * @param {SVGAElement} container The container to render the block into.
 * @param {string} parentCategory The category of this blocks parent. If no parent, than this blocks category.
 * @param {boolean} isVertical Whether the block is vertical (stack/hat/end) or horizontal (inside another block)
 * @returns {BlockComponent} The rendered component.
 */
const _renderBlock = (block, container, parentCategory, isVertical) => {
 ...
 * Renders a block, with the center of it's leftmost side located at 0, 0.
 * @param {BlockInstance} block The block to render
 * @param {SVGElement} container The container to render the block into.
