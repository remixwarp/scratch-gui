  ...
  React.useEffect(() => {
    ...
    addon.tab.traps.getBlockly()
      .then((ScratchBlocks) => {
        ...
        if (!window.ScratchBlocks) window.ScratchBlocks = ScratchBlocks;
...
export default async function ({ addon, msg }) {
  ...
  // Replaced your infinite while(true) loop with a clean, defensive DOM observer loop
  addon.tab.waitForElement('div[class*="menu-bar_file-group"] > div:last-child:not(.sa-nova)', {
    markAsSeen: true,
    reduxEvents: [
  ...
  }).then(function handleElement(elem) {
    ...
    // Continue waiting cleanly using callbacks instead of freezing async-threads
    addon.tab.waitForElement('div[class*="menu-bar_file-group"] > div:last-child:not(.sa-nova)', {
      markAsSeen: true,
      reduxEvents: [
        'scratch-gui/mode/SET_PLAYER',
