// Name: Font Manager
// ID: SPASfontManager
// Description: Add, delete, and manage fonts.
// By: SharkPool
// By: Mio <https://scratch.mit.edu/users/0znzw/>
// License: MIT

// Version V.1.1.0

/* generated l10n code */Scratch.translate.setup({"de":{"_Font Manager":"Schriftart-Verwaltung"},"fi":{"_Font Manager":"Fonttien hallinta","_[ADDED] fonts":"[ADDED] fontit","_[DATA] of font [NAME]":"fontin [NAME] [DATA]","_add font named [NAME] with fallback [BACKUP] from URL [URL]":"lisää [NAME] -niminen fontti varmistusfontilla [BACKUP] URL-osoitteesta [URL]","_add system font named [NAME] with fallback [BACKUP]":"lisää [NAME] -niminen järjestelmäfontti varmistusfontilla [BACKUP]","_all custom fonts":"kaikki omat fontit","_fallback":"varmistusfontti","_font [NAME] added?":"onko [NAME] -niminen fontti lisätty?","_format":"tiedostotyyppi","_is system":"olemassaolo järjestelmässä","_remove all fonts":"poista kaikki fontit","_remove font [NAME]":"poista fontti [NAME]","_when font is [ADDED]":"kun fontti [ADDED]","added_field_added":"lisätään","added_field_removed":"poistetaan","added_input_added":"lisätyt","added_input_removed":"poistetut"},"it":{"_Font Manager":"Gestore Caratteri","_[ADDED] fonts":"caratteri [ADDED]","_[DATA] of font [NAME]":"[DATA] del carattere [NAME]","_add font named [NAME] with fallback [BACKUP] from URL [URL]":"aggiungi carattere [NAME] con fallback [BACKUP] da URL [URL]","_add system font named [NAME] with fallback [BACKUP]":"aggiungi carattere di sistema [NAME] con alternativa [BACKUP]","_all custom fonts":"tutti i caratteri personalizzati","_fallback":"alternativa","_font [NAME] added?":"carattere [NAME] aggiunto","_format":"formato","_is system":"appartenenza ai caratteri di sistema","_remove all fonts":"rimuovi tutti i caratteri","_remove font [NAME]":"rimuovi carattere [NAME]","_when font is [ADDED]":"quando un carattere viene [ADDED]","added_field_added":"aggiunto","added_field_removed":"rimosso","added_input_added":"aggiunti","added_input_removed":"rimossi"},"ja":{"_Font Manager":"フォントマネージャー","_[ADDED] fonts":"[ADDED]フォント","_[DATA] of font [NAME]":"フォント[NAME]の[DATA]","_add font named [NAME] with fallback [BACKUP] from URL [URL]":"[NAME]という名前のフォントを[BACKUP]の代替としてURL[URL]から追加する","_add system font named [NAME] with fallback [BACKUP]":"[NAME]という名前のシステムフォントを[BACKUP]の代替として追加する","_all custom fonts":"すべてのカスタムフォント","_data: uri":"データURL","_fallback":"代替","_font [NAME] added?":"フォント[NAME]が追加された","_format":"フォーマット","_is system":"システム","_remove all fonts":"すべてのフォントを削除する","_remove font [NAME]":"フォント[NAME]を削除する","_when font is [ADDED]":"フォントが[ADDED]とき","added_field_added":"追加された","added_field_removed":"消された","added_input_added":"追加された","added_input_removed":"消された"},"ko":{"_Font Manager":"글꼴 매니저","_[ADDED] fonts":"[ADDED]된 글꼴","_[DATA] of font [NAME]":"글꼴 [NAME]의 [DATA]","_add font named [NAME] with fallback [BACKUP] from URL [URL]":"시스템 글꼴 [NAME] 추가하거나 URL의 [BACKUP](으)로 대체하기 [URL]","_add system font named [NAME] with fallback [BACKUP]":"시스템 글꼴 [NAME] 추가하거나 [BACKUP](으)로 대체하기","_all custom fonts":"사용자 지정 글꼴 목록","_data: uri":"data: URI","_fallback":"대체 글꼴","_font [NAME] added?":"글꼴 [NAME]이(가) 추가되었는가?","_format":"파일 확장자","_is system":"시스템 글꼴 여부","_remove all fonts":"모든 글꼴 제거하기","_remove font [NAME]":"글꼴 [NAME]을(를) 제거하기","_when font is [ADDED]":"글꼴이 [ADDED]되었을 때","added_field_added":"추가","added_field_removed":"제거","added_input_added":"추가","added_input_removed":"제거"},"nl":{"_Font Manager":"Lettertypes beheren","_[ADDED] fonts":"[ADDED] lettertypen","_[DATA] of font [NAME]":"[DATA] van lettertype [NAME]","_add font named [NAME] with fallback [BACKUP] from URL [URL]":"voeg lettertype genaamd [NAME] toe met reserve [BACKUP] van URL [URL]","_add system font named [NAME] with fallback [BACKUP]":"voeg systeemlettertype genaamd [NAME] toe met reserve [BACKUP]","_all custom fonts":"alle aangepaste lettertypen","_fallback":"reserve","_font [NAME] added?":"lettertype [NAME] toegevoegd?","_format":"formaat","_is system":"is systeem?","_remove all fonts":"verwijder alle lettertypen","_remove font [NAME]":"verwijder lettertype [NAME]","_when font is [ADDED]":"wanneer lettertype is [ADDED]","added_field_added":"toegevoegd","added_field_removed":"verwijderd","added_input_added":"toegevoegde","added_input_removed":"verwijderde"},"ru":{"_Font Manager":"Менеджер Шрифтов","_[ADDED] fonts":"[ADDED] шрифты","_[DATA] of font [NAME]":"[DATA] шрифта [NAME]","_add font named [NAME] with fallback [BACKUP] from URL [URL]":"добавить шрифт названный [NAME] с запасом [BACKUP] из URL-адреса [URL]","_add system font named [NAME] with fallback [BACKUP]":"добавить системный шрифт названный [NAME] с запасом  [BACKUP]","_all custom fonts":"все пользовательские шрифты","_data: uri":"данные: uri","_fallback":"запасные","_font [NAME] added?":"шрифт [NAME] добавлен?","_format":"формат","_is system":"системные","_remove all fonts":"удалить все шрифты","_remove font [NAME]":"удалить шрифт [NAME]","_when font is [ADDED]":"когда шрифт [ADDED]","added_field_added":"добавлен","added_field_removed":"удалён","added_input_added":"добавленные","added_input_removed":"удалённые"},"uk":{"_Font Manager":"Менеджер Шрифтів"},"zh-cn":{"_Font Manager":"字体管理","_[ADDED] fonts":"已[ADDED]的字体","_[DATA] of font [NAME]":"字体[NAME]的[DATA]","_add font named [NAME] with fallback [BACKUP] from URL [URL]":"从[URL]添加名为[NAME]的字体，备用字体为[BACKUP]","_add system font named [NAME] with fallback [BACKUP]":"添加名为 [NAME] 的系统字体，备用字体为 [BACKUP]","_all custom fonts":"所有字体","_data: uri":"数据URI","_fallback":"备用字体","_font [NAME] added?":"字体[NAME]已添加？","_format":"类型","_is system":"是系统字体","_remove all fonts":"移除所有字体","_remove font [NAME]":"移除字体[NAME]","_when font is [ADDED]":"当字体被[ADDED]","added_field_added":"添加","added_field_removed":"移除","added_input_added":"添加","added_input_removed":"移除"}});/* end generated l10n code */(function (Scratch) {
  "use strict";

  if (!Scratch.extensions.unsandboxed) {
    throw new Error("Font Manager must be run unsandboxed");
  }

  const extensionId = "SPASfontManager";
  const vm = Scratch.vm;
  const runtime = vm.runtime;
  const storage = runtime.storage;
  const fontManager = runtime.fontManager;

  const FONT_EXTENSIONS = [
    storage.DataFormat.TTF,
    storage.DataFormat.OTF,
    storage.DataFormat.WOFF,
    storage.DataFormat.WOFF2,
  ];

  const menuIconURI =
    "data:image/svg+xml;base64,";

  class SPASfontManager {
    constructor() {
      /** @type {string[]} */
      this.oldFonts = [];

      /** @type {string[]} */
      this.addedFonts = [];

      /** @type {string[]} */
      this.removedFonts = [];

      fontManager.on("change", () => {
        this._onchange();
      });
    }

    getInfo() {
      return {
        id: extensionId,
        name: Scratch.translate("Font Manager"),
        color1: "#2b7d6e",
        color2: "#24675b",
        color3: "#1c4e45",
        menuIconURI,
        blocks: [
          {
            opcode: "fontNames",
            blockType: Scratch.BlockType.REPORTER,
            text: Scratch.translate("all custom fonts"),
            disableMonitor: true,
          },
          {
            opcode: "fontAdded",
            blockType: Scratch.BlockType.BOOLEAN,
            text: Scratch.translate("font [NAME] added?"),
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "Comic Sans MS",
              },
            },
          },
          {
            opcode: "fontDetail",
            blockType: Scratch.BlockType.REPORTER,
            text: Scratch.translate("[DATA] of font [NAME]"),
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "Comic Sans MS",
              },
              DATA: {
                type: Scratch.ArgumentType.STRING,
                menu: "DATA",
              },
            },
          },
          "---",
          {
            opcode: "addSystemFont",
            blockType: Scratch.BlockType.COMMAND,
            text: Scratch.translate(
              "add system font named [NAME] with fallback [BACKUP]"
            ),
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "Comic Sans MS",
              },
              BACKUP: {
                type: Scratch.ArgumentType.STRING,
                menu: "FALLBACKS",
              },
            },
          },
          {
            opcode: "addCustomFont",
            blockType: Scratch.BlockType.COMMAND,
            text: Scratch.translate(
              "add font named [NAME] with fallback [BACKUP] from URL [URL]"
            ),
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "Lobster",
              },
              URL: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "https://extensions.turbowarp.org/Lobster.woff2",
              },
              BACKUP: {
                type: Scratch.ArgumentType.STRING,
                menu: "FALLBACKS",
              },
            },
          },
          {
            opcode: "removeFont",
            blockType: Scratch.BlockType.COMMAND,
            text: Scratch.translate("remove font [NAME]"),
            arguments: {
              NAME: {
                type: Scratch.ArgumentType.STRING,
                defaultValue: "Comic Sans MS",
              },
            },
          },
          {
            opcode: "removeAllFonts",
            blockType: Scratch.BlockType.COMMAND,
            text: Scratch.translate("remove all fonts"),
          },
          "---",
          {
            opcode: "whenFont",
            blockType: Scratch.BlockType.EVENT,
            text: Scratch.translate("when font is [ADDED]"),
            isEdgeActivated: false,
            arguments: {
              ADDED: {
                type: Scratch.ArgumentType.STRING,
                menu: "ADDED_FIELD",
              },
            },
          },
          {
            disableMonitor: true,
            opcode: "fontsChanged",
            blockType: Scratch.BlockType.REPORTER,
            text: Scratch.translate("[ADDED] fonts"),
            arguments: {
              ADDED: {
                type: Scratch.ArgumentType.STRING,
                menu: "ADDED_INPUT",
              },
            },
          },
        ],
        menus: {
          DATA: {
            acceptReporters: true,
            items: [
              {
                text: Scratch.translate("fallback"),
                value: "fallback",
              },
              {
                text: Scratch.translate("is system"),
                value: "is system",
              },
              {
                text: Scratch.translate("data: uri"),
                value: "data: uri",
              },
              {
                text: Scratch.translate("format"),
                value: "format",
              },
            ],
          },
          ADDED_FIELD: {
            acceptReporters: false,
            items: [
              {
                text: Scratch.translate({
                  default: "added",
                  id: "added_field_added",
                  description:
                    "Used in the context 'when font is added', an event block",
                }),
                value: "added",
              },
              {
                text: Scratch.translate({
                  default: "removed",
                  id: "added_field_removed",
                  description:
                    "Used in the context 'when font is removed', an event block",
                }),
                value: "removed",
              },
            ],
          },
          ADDED_INPUT: {
            acceptReporters: true,
            items: [
              {
                text: Scratch.translate({
                  default: "added",
                  id: "added_input_added",
                  description:
                    "Used in the context 'added fonts' for a block that reports a list of fonts that were just added",
                }),
                value: "added",
              },
              {
                text: Scratch.translate({
                  default: "removed",
                  id: "added_input_removed",
                  description:
                    "Used in the context 'removed fonts' for a block that reports a list of fonts that were just removed",
                }),
                value: "removed",
              },
            ],
          },
          FALLBACKS: {
            acceptReporters: false,
            items: [
              {
                text: "Sans Serif",
                value: "Sans Serif",
              },
              {
                text: "Serif",
                value: "Serif",
              },
              {
                text: "Handwriting",
                value: "Handwriting",
              },
              {
                text: "Marker",
                value: "Marker",
              },
              {
                text: "Curly",
                value: "Curly",
              },
              {
                text: "Pixel",
                value: "Pixel",
              },
              {
                text: "Mopeds",
                value: "Scratch",
              },
              {
                text: "中文",
                value: '"Microsoft YaHei", "微软雅黑", STXihei, "华文细黑"',
              },
              {
                text: "한국어",
                value: "Malgun Gothic",
              },
              {
                text: "日本語",
                value:
                  '"ヒラギノ角ゴ Pro W3", "Hiragino Kaku Gothic Pro", Osaka, "メイリオ", Meiryo, "ＭＳ Ｐゴシック", "MS PGothic"',
              },
            ],
          },
          FILES: {
            acceptReporters: true,
            items: FONT_EXTENSIONS.map((i) => `.${i}`),
          },
        },
      };
    }

    fontNames() {
      return JSON.stringify(fontManager.fonts.map((i) => i.family));
    }

    fontAdded(args) {
      return fontManager.hasFont(Scratch.Cast.toString(args.NAME));
    }

    fontDetail(args) {
      const name = Scratch.Cast.toString(args.NAME);
      const font = fontManager.fonts.find((item) => item.family === name);
      if (!font) return "";
      switch (Scratch.Cast.toString(args.DATA)) {
        case "is system":
          return font.system;
        case "data: uri":
          return font.asset ? font.asset.encodeDataURI() : "";
        case "format":
          return font.asset ? font.asset.dataFormat : "";
        case "fallback":
          return font.fallback;
      }
      return "";
    }

    addSystemFont(args) {
      const name = Scratch.Cast.toString(args.NAME);
      if (fontManager.isValidSystemFont(name)) {
        fontManager.addSystemFont(name, Scratch.Cast.toString(args.BACKUP));
      }
    }

    async addCustomFont(args) {
      const name = Scratch.Cast.toString(args.NAME);
      if (!fontManager.isValidCustomFont(name)) {
        return;
      }

      try {
        const response = await Scratch.fetch(Scratch.Cast.toString(args.URL));
        const arrayBuffer = await response.arrayBuffer();
        const uint8array = new Uint8Array(arrayBuffer);

        // font files should have a content-type of font/ttf, font/otf, etc.
        // if we can't detect it, we'll just assume it's ttf, browser can figure it out anyways
        const contentType = (
          response.headers.get("content-type") || ""
        ).toLowerCase();
        let dataFormat = vm.runtime.storage.DataFormat.TTF;
        for (const extension of FONT_EXTENSIONS) {
          if (contentType === `font/${extension}`) {
            dataFormat = extension;
            break;
          }
        }

        const asset = vm.runtime.storage.createAsset(
          vm.runtime.storage.AssetType.Font,
          dataFormat,
          uint8array,
          null,
          true
        );
        fontManager.addCustomFont(
          name,
          Scratch.Cast.toString(args.BACKUP),
          asset
        );
      } catch (e) {
        console.warn(e);
      }
    }

    removeFont(args) {
      const name = args.NAME;
      const index = fontManager.fonts.findIndex(
        (i) => i.family === Scratch.Cast.toString(name)
      );
      if (index !== -1) {
        fontManager.deleteFont(index);
      }
    }

    removeAllFonts() {
      fontManager.clear();
    }

    fontsChanged(args, util) {
      const added = Scratch.Cast.toString(args.ADDED);
      if (added === "added") {
        return JSON.stringify(this.addedFonts);
      } else if (added === "removed") {
        return JSON.stringify(this.removedFonts);
      } else {
        return "";
      }
    }

    _onchange() {
      this.removedFonts = [];
      this.addedFonts = [];
      for (const family of this.oldFonts) {
        if (!fontManager.hasFont(family)) {
          this.removedFonts.push(family);
        }
      }
      for (const { family } of fontManager.fonts) {
        if (!this.oldFonts.includes(family)) {
          this.addedFonts.push(family);
        }
      }
      this.oldFonts = fontManager.fonts.map((i) => i.family);

      if (this.addedFonts.length) {
        Scratch.vm.runtime.startHats(`${extensionId}_whenFont`, {
          ADDED: "added",
        });
      }
      if (this.removedFonts.length) {
        Scratch.vm.runtime.startHats(`${extensionId}_whenFont`, {
          ADDED: "removed",
        });
      }
    }
  }

  Scratch.extensions.register(new SPASfontManager());
})(Scratch);
